...
def get_next_internal_id():
    with connection.cursor() as cursor:
        cursor.execute("CREATE SEQUENCE IF NOT EXISTS achievements_internal_id_seq START WITH 1 INCREMENT BY 1")
        cursor.execute("SELECT nextval('achievements_internal_id_seq')")
...
class Achievement(models.Model):
    ...
    class Meta:
        indexes = [
            GinIndex(fields=['data'], name='achievements_data_gin'),
            models.Index(fields=['object_item'], name='achievements_object_item_idx'),
            models.Index(fields=['created_date'], name='achievements_cr_date_idx'),
            models.Index(fields=['modified_date'], name='achievements_mod_date_idx'),
            models.Index(fields=['object_item', 'object_type'], name='achievements_o_item_type_idx'),
            models.Index(fields=['project_id', 'account_id', 'user_id'], name='achievements_p_a_u_ids_idx'),
            models.Index(fields=['account_id', 'user_id'], name='achievements_a_u_ids_idx'),
            models.Index(fields=['user_id'], name='achievements_user_id_idx')
        ]
...
def set_meta(sender, instance, **kwargs):
    try:
        Achievement.objects.get(id=instance.id)
