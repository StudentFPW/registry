...
def get_next_internal_id():
    with connection.cursor() as cursor:
        cursor.execute("CREATE SEQUENCE IF NOT EXISTS ratings_internal_id_seq START WITH 1 INCREMENT BY 1")
        cursor.execute("SELECT nextval('ratings_internal_id_seq')")
...
class Rating(models.Model):
    ...
    class Meta:
        indexes = [
            GinIndex(fields=['data'], name='ratings_data_gin'),
            models.Index(fields=['object_item'], name='ratings_object_item_idx'),
            models.Index(fields=['created_date'], name='ratings_cr_date_idx'),
            models.Index(fields=['modified_date'], name='ratings_mod_date_idx'),
            models.Index(fields=['object_item', 'object_type'], name='ratings_o_item_type_idx'),
            models.Index(fields=['project_id', 'account_id', 'user_id'], name='ratings_p_a_u_ids_idx'),
            models.Index(fields=['account_id', 'user_id'], name='ratings_a_u_ids_idx'),
            models.Index(fields=['user_id'], name='ratings_user_id_idx')
        ]
...
def set_meta(sender, instance, **kwargs):
    try:
        Rating.objects.get(id=instance.id)
