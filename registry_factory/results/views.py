...
from .filters import ResultFilter, DynamicSearchFilter, CustomizedOrdering


@extend_schema(tags=['Results controllers'])
class ResultsViewset(mixins.ListModelMixin,
    ...
    @extend_schema(
        ...
        examples=[
            OpenApiExample(
                ...
                value=results_list_response_example,
                response_only=True,
            )
        ]
    )
    ...
    @extend_schema(
        examples=[
            OpenApiExample(
                ...
                value=results_bulk_create_request_example,
                request_only=True,
            ),
            OpenApiExample(
                name=f'{capitalized_app_name} bulk create response example',
                value=results_bulk_create_response_example,
                response_only=True,
            )
        ]
    )
    ...
    @extend_schema(
        examples=[
            OpenApiExample(
                ...
                value=results_bulk_put_request_example,
                request_only=True,
            ),
            OpenApiExample(
                name=f'{capitalized_app_name} bulk put response example',
                value=results_bulk_put_response_example,
                response_only=True,
            )
        ]
    )
    def bulk_update(self, request, *args, **kwargs):
        ...
        instances = Result.objects.filter(id__in=ids).order_by('id')  # We only take instances that need to be updated
    ...
    @extend_schema(
        examples=[
            OpenApiExample(
                ...
                value=results_bulk_patch_request_example,
                request_only=True,
            ),
            OpenApiExample(
                name=f'{capitalized_app_name} bulk patch response example',
                value=results_bulk_patch_response_example,
                response_only=True,
            )
        ]
    )
    ...
    def bulk_deactivate(self, request, *args, **kwargs):
        ...
        objs = Result.objects.filter(id__in=id_list)
        ...
        if len(id_list) == len(objs):
            ...
            return Response(data={"message": f"{len(objs)} results deactivated out of {len(id_list)}. "
...
@extend_schema(tags=['Result controllers'])
class ResultViewset(mixins.RetrieveModelMixin,
    ...
    @extend_schema(
        examples=[
            OpenApiExample(
                ...
                value=results_single_retrieve_response_example,
                response_only=True,
            )
        ]
    )
    ...
    @extend_schema(
        examples=[
            OpenApiExample(
                ...
                value=results_single_put_request_example,
                request_only=True,
            ),
            OpenApiExample(
                name=f'{capitalized_app_name} single put response example',
                value=results_single_put_response_example,
                response_only=True,
            )
        ]
    )
    ...
    @ extend_schema(
        examples=[
            OpenApiExample(
                ...
                value=results_single_create_request_example,
                request_only=True,
            ),
            OpenApiExample(
                name=f'{capitalized_app_name} single create response example',
                value=results_single_create_response_example,
                response_only=True,
            )
        ]
    )
    ...
    @extend_schema(
        examples=[
            OpenApiExample(
                ...
                value=results_single_patch_request_example,
                request_only=True,
            ),
            OpenApiExample(
                name=f'{capitalized_app_name} single patch response example',
                value=results_single_patch_response_example,
                response_only=True,
            )
        ]
    )
    ...
    @extend_schema(responses={
            200: OpenApiResponse(
                inline_serializer(
                    name='ResultDeactivatedResponse',
                    fields={
                        'message': serializers.CharField(default="Result deactivated"),
                    }
                ),
                description="Deactivated",
            )
    ...
    def deactivate(self, request, *args, **kwargs):
        ...
        instance.save()
        return Response(data={"message": "Result deactivated"},
                        status=status.HTTP_200_OK)
